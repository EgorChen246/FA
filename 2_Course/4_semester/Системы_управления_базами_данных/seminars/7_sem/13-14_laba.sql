...
-- Блок 2
-- Задание 1. Функция с составным типом booking_type. возвращается уменьшенное на 30% значение total_amount.
...
-- Задание 2. Процедура update_contact_data, обнавляет данные
...
-- Задание 3. Функция calculate_difference (VARIADIC), разность переданных аргументов
...
-- Задание 4. Функция generate_ticket_flights_table, пораждаем таблицу
SELECT * FROM flights;
SELECT * FROM ticket_flights;
SELECT * FROM tickets;
...
-- Задание 5. Два варианта функций (SETOF и SETOF RECORD), выводим строки в которых fare_conditions = fare_cond.
...
-- Блок 3
-- 1. Переменная на четырёх уровнях вложенности. Переменная с одним и тем же именем объявляется и используется
...
-- 2. Динамические команды (SELECT, UPDATE, DELETE), в которые передаются параметры и используются запросы:
SELECT * FROM flights;
SELECT * FROM ticket_flights;
SELECT * FROM tickets;
...
-- SELECT * FROM find_seat_pairs_by_last_char();
CALL find_seat_pairs_by_last_char();


-- 5. Курсор с параметром (flights + aircrafts), выведите все строки, в которых модель самолета равна заданной модели
SELECT * FROM aircrafts_data;
SELECT * FROM flights;
