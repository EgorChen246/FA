...
DROP TABLE name_product CASCADE;
DROP TABLE storage CASCADE;
DROP TABLE worker CASCADE;
DROP TABLE storage_goods CASCADE;
DROP TABLE sale CASCADE;
DROP TABLE product CASCADE;
DROP TABLE young_employees CASCADE;
...
SELECT * FROM worker;
SELECT name_company FROM firm;

-- Задание

CREATE TABLE young_employees AS SELECT 
	w.surname,
	w.name_,
	w.birth_date,
	w.title,
...
LEFT JOIN firm f ON w.code_firm = f.code_firm WHERE EXTRACT(YEAR FROM w.birth_date) >= 1989;

UPDATE young_employees SET telephone = '345-76-98' WHERE surname = 'Чижиков';

SELECT * FROM young_employees ORDER BY birth_date;