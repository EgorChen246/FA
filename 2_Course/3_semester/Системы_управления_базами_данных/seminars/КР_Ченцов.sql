...
SELECT 
	w.surname AS "Фамилия",
	w.name_ AS "Имя",
	w.title AS "Должность",
	f.name_company AS "Название фирмы",
	f.city AS "Город фирмы"
FROM worker w
 
JOIN firm f ON w.code_firm = f.code_firm
JOIN sale s ON w.code_worker = s.code_worker
JOIN product p ON s.code_product = p.code_product
WHERE p.country IN ('Китай', 'Япония') AND f.city IN ('Москва', 'Тверь', 'Омск')

GROUP BY w.code_worker, f.code_firm, w.surname, w.name_, w.title, f.name_company, f.city
ORDER BY w.surname, f.name_company;

-- 2

SELECT 
	EXTRACT(MONTH FROM s.date_time_sale) AS "Месяц",
	SUM(p.price * s.quantity * (1 - s.discount)) AS "Общая сумма"
FROM sale s	
JOIN product p ON s.code_product = p.code_product
WHERE EXTRACT(YEAR FROM s.date_time_sale) = 2016

GROUP BY EXTRACT(MONTH FROM s.date_time_sale)
ORDER BY "Месяц";
...
JOIN name_product np ON np.code_name = p.code_name
WHERE p.code_product NOT IN (SELECT DISTINCT s.code_product FROM sale s)
...
SELECT 
	np._name AS "Наименование",
	p.brand AS "Марка",
	p.date_production AS "Дата производства",
	EXTRACT(YEAR FROM AGE(CURRENT_DATE, p.date_production)) AS "Возраст в годах"
FROM product p

JOIN name_product np ON p.code_name = np.code_name
WHERE p.date_production IS NOT NULL

ORDER BY EXTRACT(YEAR FROM AGE(CURRENT_DATE, p.date_production)) DESC;